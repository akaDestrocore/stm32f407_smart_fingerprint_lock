...
void ST7735_GPIO_Init(void)
{
	...
	//CS and RES
	SPI_Pins.pGPIOx = GPIOC;
	SPI_Pins.GPIO_Config.PinMode = GPIO_MODE_OUTPUT;
	SPI_Pins.GPIO_Config.PinOPType = GPIO_OUTPUT_PP;
	SPI_Pins.GPIO_Config.PinPuPdControl = GPIO_PIN_NO_PUPD;
	SPI_Pins.GPIO_Config.PinSpeed = GPIO_SPEED_LOW;

	SPI_Pins.GPIO_Config.PinNumber = PIN_0;		//CS pin
	GPIO_Init(&SPI_Pins);

	SPI_Pins.GPIO_Config.PinNumber = PIN_2;		//RES pin
	GPIO_Init(&SPI_Pins);
	...
	SPI_Pins.GPIO_Config.PinPuPdControl = GPIO_PIN_NO_PUPD;
	SPI_Pins.GPIO_Config.PinSpeed = GPIO_SPEED_LOW;
	SPI_Pins.GPIO_Config.PinNumber = PIN_0;
	GPIO_Init(&SPI_Pins);
	...
	SPI_Pins.GPIO_Config.PinPuPdControl = GPIO_PIN_NO_PUPD;
	SPI_Pins.GPIO_Config.PinSpeed = GPIO_SPEED_LOW;
	SPI_Pins.GPIO_Config.PinNumber = PIN_2;
	GPIO_Init(&SPI_Pins);
	...
	//SLCK and MOSI
	SPI_Pins.pGPIOx = GPIOA;
	SPI_Pins.GPIO_Config.PinMode = GPIO_MODE_AF;
	SPI_Pins.GPIO_Config.PinAltFuncMode = 5;
	SPI_Pins.GPIO_Config.PinOPType = GPIO_OUTPUT_PP;
	SPI_Pins.GPIO_Config.PinPuPdControl = GPIO_PIN_NO_PUPD;
	SPI_Pins.GPIO_Config.PinSpeed = GPIO_SPEED_FAST;

	SPI_Pins.GPIO_Config.PinNumber = PIN_5;		//SLCK pin
	GPIO_Init(&SPI_Pins);

	SPI_Pins.GPIO_Config.PinNumber = PIN_7;		//MOSI pin
	GPIO_Init(&SPI_Pins);
}
...
static void ST7735_WriteData(SPI_Handle_t *pSPIHandle,uint8_t* buff, size_t buff_size)
{
	...
	while(pSPIHandle->TxState == SPI_BUSY_IN_TX);
#else
...
static void ST7735_ExecuteCommandList(const uint8_t *addr)
{
    ...
    while(numCommands--)
    {
        ...
        if(numArgs)
        {
            ST7735_WriteData(&mySPIhandler,(uint8_t*)addr, numArgs);
            addr += numArgs;
        }
...
static void ST7735_SetAddressWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1)
{
    ...
    ST7735_WriteData(&mySPIhandler, data, sizeof(data));
...
void ST7735_Print(uint16_t x, uint16_t y, const char* str, float value , FontDef_t font, uint16_t color, uint16_t bgcolor)
{
	TFT_CS_L();
...
void ST7735_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t* data)
{
    ...
    TFT_CS_L();
    ST7735_SetAddressWindow(x, y, x+w-1, y+h-1);
    ST7735_WriteData(&mySPIhandler, (uint8_t*)data, sizeof(uint16_t)*w*h);
    TFT_CS_H();
}
...
void ST7735_SetRotation(uint8_t m)
{
  ...
  switch (_value_rotation)
  {
    case 0:
    {
    	...
    	ST7735_WriteData(&mySPIhandler,&d_r, sizeof(d_r));
        _width  = ST7735_WIDTH;
